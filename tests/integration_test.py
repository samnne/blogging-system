...
class IntegrationTest(TestCase):
    ...
    def test_login_logout(self):
        ...
        with self.assertRaises(
            InvalidLogoutException, msg="log out only after being logged in"
        ):
            self.controller.logout()

        with self.assertRaises(
            InvalidLoginException, msg="login in with incorrect username"
        ):
            self.controller.login("incorrectuser", "123456")

        with self.assertRaises(
            InvalidLoginException, msg="login in with incorrect password"
        ):
        ...
        with self.assertRaises(
            DuplicateLoginException, msg="cannot login again while still logged in"
        ):
            self.controller.login("user", "123456")
        ...
        self.assertTrue(
            self.controller.login("ali", "@G00dPassw0rd"), "another user logging in"
        )
    ...
    def test_create_search_blog(self):
        ...
        expected_blog_1 = Blog(
            1111114444, "Short Journey", "short_journey", "short.journey@gmail.com"
        )
        expected_blog_2 = Blog(
            1111115555, "Long Journey", "long_journey", "long.journey@gmail.com"
        )
        expected_blog_3 = Blog(
            1111112000, "Long Trip", "long_trip", "long.trip@gmail.com"
        )

        # cannot do search and create operations without logging in
        with self.assertRaises(
            IllegalAccessException, msg="cannot search blog without logging in"
        ):
            self.controller.search_blog(1111114444)
        with self.assertRaises(
            IllegalAccessException, msg="cannot create blog without logging in"
        ):
        ...
        # add one blog
        self.assertTrue(self.controller.login("user", "123456"), "login correctly")
        actual_blog = self.controller.create_blog(
            1111114444, "Short Journey", "short_journey", "short.journey@gmail.com"
        )
        ...
        self.assertEqual(
            expected_blog_1,
            actual_blog,
            "Short Journey blog was created and their data are correct",
        )
        ...
        self.assertEqual(
            expected_blog_1,
            actual_blog,
            "Short Journey blog was created, retrieved and its data are correct",
        )

        # should not allow to create another blog with same id
        with self.assertRaises(
            IllegalOperationException,
            msg="cannot add a blog with an ID that is already registered",
        ):
            self.controller.create_blog(
                1111114444, "Long Journey", "long_journey", "long.journey@gmail.com"
            )
        ...
        self.assertEqual(
            expected_blog_2,
            actual_blog,
            "second blog, Long Journey, was created and its data are correct",
        )
        ...
        self.assertEqual(
            expected_blog_2,
            actual_blog,
            "second blog, Long Journey, was created, retrieved and its data are correct",
        )
        ...
        self.assertEqual(
            expected_blog_3,
            actual_blog,
            "Long Trip blog was created and its data are correct",
        )
        ...
        self.assertEqual(
            expected_blog_3,
            actual_blog,
            "third blog, Long Trip, was created, retrieved and its data are correct",
        )
        ...
        self.assertIsNotNone(
            actual_blog,
            "blog created and retrieved cannot be null, regardless of search order",
        )
        self.assertEqual(
            expected_blog_2,
            actual_blog,
            "Long Journey blog was created, retrieved and its data are correct regardless of search order",
        )
        actual_blog = self.controller.search_blog(1111114444)
        self.assertIsNotNone(
            actual_blog,
        ...
        self.assertEqual(
            expected_blog_1,
            actual_blog,
            "Short Journey blog was created, retrieved and its data are correct regardless of search order",
        )
    ...
    def test_retrieve_blogs(self):
        ...
        expected_blog_4 = Blog(
            1111116666, "Short Trip", "short_trip", "short.trip@gmail.com"
        )
        expected_blog_5 = Blog(
            1111117777, "Boring Blog", "boring_blog", "boring.blog@gmail.com"
        )

        # cannot do operation without logging in
        with self.assertRaises(
            IllegalAccessException, msg="cannot retrieve blogs without logging in"
        ):
        ...
        self.assertEqual(
            expected_blog_2, actual_blog, "retrieved blog in the list is Long Journey"
        )
        ...
        self.assertEqual(
            2,
            len(retrieved_list),
            "retrieved list of blogs with Journey keyword has size 2",
        )
        self.assertEqual(
            expected_blog_1,
            retrieved_list[0],
            "first blog in the retrieved list is Short Journey",
        )
        self.assertEqual(
            expected_blog_2,
            retrieved_list[1],
            "second blog in the retrieved list is Long Journey",
        )
    ...
    def test_update_blog(self):
        ...
        # cannot do operation without logging in
        with self.assertRaises(
            IllegalAccessException, msg="cannot update blog without logging in"
        ):
        ...
        with self.assertRaises(
            IllegalOperationException,
            msg="cannot update blog with an ID that is not registered",
        ):
            self.controller.update_blog(
                1111114444,
                1111114444,
        ...
        self.assertNotEqual(
            expected_blog_1,
            actual_blog,
            "blog has updated data, cannot be equal to the original data",
        )
        expected_blog_3a = Blog(
            1111114444, "Short Travel", "short_travel", "short.travel@gmail.com"
        )
        self.assertEqual(
            expected_blog_3a,
            actual_blog,
            "blog was updated, its data have to be updated and correct",
        )

        # update one blog, and change the Blog key (id) as well
        self.assertTrue(
            self.controller.update_blog(
                1111117777, 1111118888, "Cool Blog", "cool_blog", "cool.blog@gmail.com"
            ),
        ...
        expected_blog_5a = Blog(
            1111118888, "Cool Blog", "cool_blog", "cool.blog@gmail.com"
        )
        self.assertEqual(
            expected_blog_5a,
            actual_blog,
    ...
    def test_delete_blog(self):
        ...
        # cannot do operation without logging in
        with self.assertRaises(
            IllegalAccessException, msg="cannot delete blog without logging in"
        ):
        ...
        with self.assertRaises(
            IllegalOperationException,
            msg="cannot delete blog when no blogs are registered",
        ):
            self.controller.delete_blog(1111114444)
        ...
        with self.assertRaises(
            IllegalOperationException,
            msg="cannot delete blog with an ID that is not registered",
        ):
        ...
        self.assertTrue(
            self.controller.delete_blog(1111114444),
            "delete blog from the start of the collection",
        )
        ...
        self.assertIsNone(
            self.controller.search_blog(1111114444),
            "deleted blog cannot be found in the system",
        )

        # delete one blog at the middle of the collection
        self.assertTrue(
            self.controller.delete_blog(1111112000),
            "delete blog from the middle of the collection",
        )
        ...
        self.assertIsNone(
            self.controller.search_blog(1111112000),
            "deleted blog cannot be found in the system",
        )
        ...
        self.assertTrue(
            ...
            "delete blog from the end of the collection",
        )
        ...
        self.assertIsNone(
            self.controller.search_blog(1111117777),
            "deleted blog cannot be found in the system",
        )
    ...
    def test_list_blogs(self):
        ...
        # cannot do operation without logging in
        with self.assertRaises(
            IllegalAccessException, msg="cannot list blogs without logging in"
        ):
        ...
        self.assertEqual(
            expected_blog_1,
            blogs_list[0],
            "Short Journey blog is the only one in the list of blogs",
        )
        ...
        self.assertEqual(
            expected_blog_1, blogs_list[0], "blog 1 is the first in the list of blogs"
        )
        self.assertEqual(
            expected_blog_2, blogs_list[1], "blog 2 is the second in the list of blogs"
        )
        self.assertEqual(
            expected_blog_3, blogs_list[2], "blog 3 is the third in the list of blogs"
        )
        self.assertEqual(
            expected_blog_4, blogs_list[3], "blog 4 is the fourth in the list of blogs"
        )
        self.assertEqual(
            expected_blog_5, blogs_list[4], "blog 5 is the fifth in the list of blogs"
        )
        ...
        self.assertEqual(
            expected_blog_2, blogs_list[0], "blog 2 is the first in the list of blogs"
        )
        self.assertEqual(
            expected_blog_4, blogs_list[1], "blog 4 is the second in the list of blogs"
        )
    ...
    def test_set_get_current_blog(self):
        ...
        with self.assertRaises(
            IllegalAccessException, msg="cannot get current blog without logging in"
        ):
            self.controller.get_current_blog()
        with self.assertRaises(
            IllegalAccessException, msg="cannot set current blog without logging in"
        ):
            self.controller.set_current_blog(1111110001)
        with self.assertRaises(
            IllegalAccessException, msg="cannot unset current blog without logging in"
        ):
        ...
        self.assertIsNone(
            self.controller.get_current_blog(),
            "cannot get current blog without setting them first",
        )

        # cannot set a non-existent blog to be the current blog
        with self.assertRaises(
            IllegalOperationException,
            msg="cannot set non-existent blog as the current blog",
        ):
            self.controller.set_current_blog(1111110001)
        ...
        self.assertEqual(
            expected_blog_3, actual_current_blog, "expected current blog is blog 3"
        )

        # cannot delete the current blog, unset current blog first
        with self.assertRaises(
            IllegalOperationException, msg="cannot delete the current blog"
        ):
            self.controller.delete_blog(1111112000)

        # cannot update the current blog, unset current blog first
        with self.assertRaises(
            IllegalOperationException, msg="cannot update the current blog"
        ):
            self.controller.update_blog(
                1111112000,
                1111112000,
        ...
        with self.assertRaises(
            IllegalAccessException, msg="cannot get current blog after logging out"
        ):
            self.controller.get_current_blog()

    def test_create_post(self):
        # some posts that may be created
        expected_post_1 = Post(
            1, "Starting my journey", "Once upon a time\nThere was a kid..."
        )
        expected_post_2 = Post(
            2, "Continuing my journey", "Along the way...\nThere were challenges."
        )
        expected_post_3 = Post(
            3, "Finishing my journey", "And that was it.\nEnd of story."
        )

        # cannot do search and create operations without logging in
        with self.assertRaises(
            IllegalAccessException,
            msg="cannot search post from a blog without logging in",
        ):
            self.controller.search_post(1)
        with self.assertRaises(
            IllegalAccessException, msg="cannot add post in a blog without logging in"
        ):
            self.controller.create_post(
                "Starting my journey", "Once upon a time\nThere was a kid..."
            )
        ...
        with self.assertRaises(
            NoCurrentBlogException,
            msg="cannot search post without a valid current blog",
        ):
            self.controller.search_post(1)
        with self.assertRaises(
            NoCurrentBlogException, msg="cannot add post without a valid current blog"
        ):
            self.controller.create_post(
                "Starting my journey", "Once upon a time\nThere was a kid..."
            )
        ...
        # add one post
        actual_post = self.controller.create_post(
            "Starting my journey", "Once upon a time\nThere was a kid..."
        )
        ...
        self.assertEqual(
            expected_post_1, actual_post, "post 1 was created and its data are correct"
        )
        ...
        self.assertEqual(
            expected_post_1,
            actual_post,
            "post 1 was created, retrieved and its data are correct",
        )

        # add a second post
        actual_post = self.controller.create_post(
            "Continuing my journey", "Along the way...\nThere were challenges."
        )

        self.assertIsNotNone(actual_post, "post 2 was created and is valid")
        self.assertEqual(
            expected_post_2, actual_post, "post 2 was created and its data are correct"
        )
        ...
        self.assertEqual(
            expected_post_2,
            actual_post,
            "post 2 was created, retrieved and its data are correct",
        )

        # add a third post
        actual_post = self.controller.create_post(
            "Finishing my journey", "And that was it.\nEnd of story."
        )

        self.assertIsNotNone(actual_post, "post 3 was created and is valid")
        self.assertEqual(
            expected_post_3, actual_post, "post 3 was created and its data are correct"
        )
        ...
        self.assertEqual(
            expected_post_3,
            actual_post,
            "post 3 was created, retrieved and its data are correct",
        )
        ...
        self.assertIsNotNone(
            actual_post,
            "post created and retrieved cannot be null regardless of search order",
        )
        self.assertEqual(
            expected_post_2,
            actual_post,
            "post 2 was created, retrieved and its data are correct regardless of search order",
        )
        actual_post = self.controller.search_post(1)
        self.assertIsNotNone(
            actual_post,
        ...
        self.assertEqual(
            expected_post_1,
            actual_post,
            "post 1 was created, retrieved and its data are correct regardless of search order",
        )
    ...
    def test_retrieve_posts(self):
        ...
        expected_post_2 = Post(
            2, "Second step", "Before one could think,\nA storm stroke."
        )
        expected_post_3 = Post(
            3, "Continuing my journey", "Along the way...\nThere were challenges."
        )
        expected_post_4 = Post(
            4, "Fourth step", "When less expected,\nAll worked fine."
        )
        expected_post_5 = Post(
            5, "Finishing my journey", "And that was it.\nEnd of story."
        )

        # cannot do operation without logging in
        with self.assertRaises(
            IllegalAccessException,
            msg="cannot retrieve posts from a blog without logging in",
        ):
        ...
        with self.assertRaises(
            NoCurrentBlogException,
            msg="cannot retrieve posts without a valid current blogs",
        ):
            self.controller.retrieve_posts("journey")
        ...
        self.controller.create_post(
            "Second step", "Before one could think,\nA storm stroke."
        )
        self.controller.create_post(
            "Continuing my journey", "Along the way...\nThere were challenges."
        )
        self.controller.create_post(
            "Fourth step", "When less expected,\nAll worked fine."
        )
        self.controller.create_post(
            "Finishing my journey", "And that was it.\nEnd of story."
        )
        ...
        self.assertEqual(
            expected_post_2, actual_post, "retrieved post in the list is post 2"
        )
        ...
        self.assertEqual(
            3,
            len(retrieved_list),
            "retrieved list of journey posts from Short Journey blog has size 3",
        )
        self.assertEqual(
            expected_post_1,
            retrieved_list[0],
            "first retrieved post in the list is post 1",
        )
        self.assertEqual(
            expected_post_3,
            retrieved_list[1],
            "second retrieved post in the list is post 2",
        )
        self.assertEqual(
            expected_post_5,
            retrieved_list[2],
            "third retrieved post in the list is post 4",
        )
    ...
    def test_update_post(self):
        ...
        with self.assertRaises(
            IllegalAccessException,
            msg="cannot update post from a blog without logging in",
        ):
            self.controller.update_post(
                3,
        ...
        with self.assertRaises(
            NoCurrentBlogException,
            msg="cannot update post without a valid current blog",
        ):
            self.controller.update_post(
                3,
                "Continuing the journey",
        ...
        self.assertNotEqual(
            expected_post_3,
            actual_post,
            "post has updated data, cannot be equal to the original data",
        )
        expected_post_3a = Post(
            3, "Continuing the journey", "Along the way...\nThere were new challenges."
        )
        self.assertEqual(
            expected_post_3a,
            actual_post,
            "blog was updated, their data has to be updated and correct",
        )
        ...
        self.assertTrue(
            self.controller.update_post(
                5, "Finishing my travel", "And that was it.\nEnd of travel."
            ),
            "update blog's post",
        )
    ...
    def test_delete_post(self):
        ...
        with self.assertRaises(
            IllegalAccessException,
            msg="cannot delete post from a blog without logging in",
        ):
        ...
        with self.assertRaises(
            NoCurrentBlogException,
            msg="cannot delete post without a valid current blog",
        ):
            self.controller.delete_post(3)
        ...
        self.assertFalse(
            self.controller.delete_post(3),
            "cannot delete post when there are no posts for that blog in the system",
        )
    ...
    def test_list_posts(self):
        ...
        with self.assertRaises(
            IllegalAccessException,
            msg="cannot list post from a blog without logging in",
        ):
        ...
        # cannot do operation without a valid current blog
        with self.assertRaises(
            NoCurrentBlogException, msg="cannot list posts without a valid current blog"
        ):
            self.controller.list_posts()
        ...
        self.assertEqual(
            expected_post_5, posts_list[0], "post 5 is the first in the list of blogs"
        )
        self.assertEqual(
            expected_post_4, posts_list[1], "post 4 is the second in the list of blogs"
        )
        self.assertEqual(
            expected_post_3, posts_list[2], "post 3 is the third in the list of blogs"
        )
        self.assertEqual(
            expected_post_2, posts_list[3], "post 2 is the fourth in the list of blogs"
        )
        self.assertEqual(
            expected_post_1, posts_list[4], "post 1 is the fifth in the list of blogs"
        )
        ...
        self.assertEqual(
            expected_post_4, posts_list[0], "post 4 is the first in the list of posts"
        )
        self.assertEqual(
            expected_post_2, posts_list[1], "post 2 is the second in the list of posts"
        )
